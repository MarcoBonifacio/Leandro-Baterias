...
export async function createOrderInSupabase(
  ...
  try {
    ...
    const orderRecord: any = {
      ...
      user_id: userId, // Guardar id del usuario logueado en Supabase
...
export async function addProductToSupabase(newProduct: Omit<BatteryProduct, 'id'>): Promise<BatteryProduct> {
  try {
    ...
    if (error) {
      console.warn('Primary insert failed. Retrying with lowercase imageurl & warrantyMonths...', error.message);
      const { image_url, warranty_months, ...rest } = productRecord;
      const fallbackRecord = {
        ...rest,
        image_url: image_url || '',
        imageurl: image_url || '',
        ...
        warrantyMonths: warranty_months || 12,
      };
      const { error: errorFallback } = await supabase.from('products').insert(fallbackRecord);
      if (errorFallback) {
        throw errorFallback;
      }
    }
...
export async function updateProductInSupabase(productId: string, updatedFields: Partial<BatteryProduct>): Promise<BatteryProduct> {
  try {
    ...
    if (updatedFields.warrantyMonths !== undefined) {
      ...
      updatePayload.warrantyMonths = Number(updatedFields.warrantyMonths);
    }
    ...
    if (updatedFields.imageUrl !== undefined) {
      ...
      updatePayload.imageUrl = updatedFields.imageUrl;
      updatePayload.imageurl = updatedFields.imageUrl;
    }
